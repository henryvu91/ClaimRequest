...
//    Check email is existed
    ...
    @Query("SELECT new com.vn.dto.StaffIdNameDto(s.id,s.name) FROM Staff s")
    List<StaffIdNameDto> findAllStaffName();

//    Get the detail information of staff

    @Query("SELECT new com.vn.dto.StaffViewDetailDto(s.id, s.email, s.name, s.salary, s.departmentByDepartmentId.name, s.roleByRoleId.name) FROM Staff s WHERE s.id=:id")
